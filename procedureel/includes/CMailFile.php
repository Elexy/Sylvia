...
class CMailFile {
	...
	function CMailFile($subject,$to,$from,$msg,$filename,$mimetype = "application/octet-stream", $mime_filename = false, $body_mimetype = "text/plain") {
		$this->subject = $subject;
		$this->addr_to = $to;
		$this->smtp_headers = $this->write_smtpheaders($from);
		$this->text_body = $this->write_body($msg, $body_mimetype);
...
// added -- to notify email client attachment is done
	...
	function encode_file($sourcefile) {
		...
		if (is_readable($sourcefile)) {
			if ($fd = fopen($sourcefile, "r")) {
                ...
                echo "Cann't open file $sourcefile";
            }
		} else {
            echo "Cann't read file $sourcefile";
        }
		return $encoded;
	}
	...
	function write_body($msgtext, $body_mimetype) {
		$out = "--" . $this->mime_boundary . "\n";
		$out = $out . "Content-Type:" . $body_mimetype . "; charset=\"us-ascii\"\n\n";
		$out = $out . $msgtext . "\n";
		return $out;
	}
